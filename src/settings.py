...
class ProviderRegistry:
    ...
    LLM_CONFIGS = {
        ...
        "ollama": {
            ...
            "class": "Ollama",
            "defaults": {"model": "llama2", "temperature": 0.1, "base_url": "http://localhost:11434"},
    ...
    @classmethod
    def _create_instance(cls, provider: str, config_dict: Dict, user_kwargs: Dict) -> Any:
        """Generic factory method to create provider instances"""
        if provider not in config_dict:
            raise ValueError(f"Unsupported provider: {provider}. Available: {list(config_dict.keys())}")
        ...
        try:
            ...
            raise ImportError(f"{provider} not installed. Run: pip install {package}") from e
...
def init_settings(
    ...
    **global_settings
    ...
    Settings.embed_model = ProviderRegistry.get_embedding_model(embedding_provider, **(embedding_config or {}))
    
    # Configure global settings with environment fallbacks
    Settings.chunk_size = global_settings.get("chunk_size", int(os.getenv("CHUNK_SIZE", "512")))
    Settings.chunk_overlap = global_settings.get("chunk_overlap", int(os.getenv("CHUNK_OVERLAP", "50")))
